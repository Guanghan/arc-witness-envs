...
from witness_grid import (
    ...
    COLOR_BLACK, SQUARE_A, SQUARE_B, SQUARE_C, ERASER_COLOR,
    TRI_COLOR, POLY_COLOR,
)
...
class Tw13(ARCBaseGame):
    ...
    def _load_json_levels() -> Optional[list]:
        try:
            import json
            levels_path = os.path.join(_code_dir, "levels", "tw13_levels.json")
        ...
        if json_entries:
            ...
            for entry in json_entries:
                ...
                config = {
                    ...
                    "dots": [tuple(d) for d in cfg.get("dots", [])],
            ...
            level_configs = [
                {
                    ...
                    "dots": [],
                    "tetris": {
                        (0, 0): {"shape": [(0, 0), (1, 0)], "rotated": False, "negative": False},
                    },
                },
            ]
    ...
    def on_set_level(self, level: Level) -> None:
        ...
        self._dots = [tuple(d) for d in data.get("dots", [])]
